...
public class ReviewController {
	...
	@PostMapping
	public ResponseEntity<Map<String, String>> addReview(@PathVariable("festivalId") Long festivalId,
			@RequestBody ReviewCreateRequest request, @AuthenticationPrincipal UserDetails userDetails) {
		if (userDetails == null) {
	...
	public ResponseEntity<Map<String, String>> deleteReview(@PathVariable("festivalId") Long festivalId,
			@PathVariable("reviewId") Long reviewId, @AuthenticationPrincipal UserDetails userDetails) {
		if (userDetails == null) {
			return ResponseEntity.status(401).body(Map.of("message", "로그인이 필요합니다!"));
		}
