...
GGNOW는 경기도 행사·축제 정보를 카테고리별로 조회하고, 사용자가 관심 행사를 즐겨찾기하거나 후기를 작성할 수 있는 웹 서비스입니다.

이 프로젝트는 단순 CRUD 구현보다 **외부 API 데이터 정제, 내부 식별자 설계, 인증/인가, 사용자 데이터 연결, 배포 환경 문제 해결**에 중점을 두고 개발했습니다.

### 이 프로젝트에서 중점적으로 구현한 부분

- API 데이터를 주기적으로 수집하여 DB에 적재
- 서비스 화면은 외부 API가 아닌 내부 DB 데이터를 조회하도록 설계
- 외부 API 장애 상황에서도 저장된 데이터를 기반으로 서비스 제공 가능
- 제목, 날짜, 시간, 주소, 참가비를 조합한 fingerprint 기반 중복 제거 로직 구현
- 외부 API 데이터와 내부 서비스 식별자를 분리하기 위해 `identityKey`와 `festivalId` 구조 설계
- Spring Security 기반 로그인/로그아웃 및 BCrypt 비밀번호 암호화 적용
- 리뷰 수정/삭제 시 작성자 본인만 가능하도록 권한 검증 구현
- 즐겨찾기, 리뷰, 마이페이지 기능을 내부 행사 식별자와 연결
- Render, Aiven DB, Docker, Railway를 활용한 배포 경험
- JUnit5, Mockito 기반 단위 테스트 작성 및 핵심 비즈니스 로직 검증
...
### 1. 행사 목록 조회
- Scheduler가 API에서 행사 데이터를 수집합니다.
- 수집된 데이터를 DB에 저장합니다.
- 행사 목록 페이지에는 외부 API가 아닌 DB에 적재된 데이터를 조회하여 출력합니다.
- API 장애 발생 시에도 기존에 저장된 데이터를 계속 제공할 수 있습니다.
...
- 마이페이지에서 본인이 작성한 리뷰를 확인할 수 있습니다.
...
### 8. 행사 데이터 자동 동기화
- Spring Scheduler를 활용하여 외부 API 데이터를 주기적으로 수집합니다.
- 수집한 행사 데이터를 DB에 저장합니다.
- 서비스 화면은 DB에 저장된 데이터로 조회합니다.
- 따라서 API에 장애가 발생해도 기존 데이터 기반으로 서비스가 유지됩니다.
- 행사 데이터는 SHA-256 기반 Identity Key를 이용해 중복 저장을 방지합니다.
- 마지막 성공 동기화 시간을 기록하여 데이터 최신 여부를 확인할 수 있습니다.
---
...
## 테스트
프로젝트를 처음 개발할 때에는 기능 구현과 배포에 집중했지만, 유지보수 과정에서 핵심 비즈니스 로직의 안정성을 검증하기 위해 JUnits와 Mockito를 활용한 단위테스트를 추가했습니다.
...
### UserService Test
회원가입 과정에서 발생할 수 있는 주요 검증 로직을 테스트 했습니다.

- 회원가입 성공
- 아이디 중복 검증

### ReviewSerivce Test
행사 리뷰 기능의 핵심 로직과 권한 검증을 테스트 했습니다.
...
### FestivalService Test
행사 식별 및 검색에 사용되는 핵심 로직을 테스트 했습니다.

- normalize() 문자열 정규화 검증
- null 입력 처리 검증
- 동일한 행사 정보에 대한 identityKey 생성 검증
- 날짜 변경 시 다른 identityKey 생성 검증
- 주소 변경 시 다른 identityKey 생성 검증

### Favorite 기능 테스트
즐겨찾기 기능의 기본 동작을 테스트 했습니다.
...
총 14개의 단위 테스트를 작성했으며 모든 테스트가 성공하는 것을 확인했습니다.

테스트를 통해 회원가입, 리뷰권한 처리, 즐겨찾기 처리, 행사 식별 로직이 의도한 대로 동작하는지 검증했습니다.

또한 고의로 실패를 일으켜봄으로서 테스트가 정상 작동하는지 확인했습니다.
...
### 2. 외부 API 데이터 처리
- 공공 문화행사 API를 활용하여 행사 정보를 수집하도록 구현했습니다.
- 기존에는 경기데이터드림(Open API)의 XML 응답을 사용했으나, 데이터 제공 중단 이슈 발생 후 경기문화재단(GGC) Open API의 JSON 응답 구조로 변경했습니다.
...
### 5. 데이터 안정성 개선
- 외부 API 서비스 중단 경험을 계기로 데이터 의존성을 줄이는 구조를 고민했습니다.
- Spring Scheduler를 사용해 행사 데이터를 주기적으로 DB에 적재하도록 구현했습니다.
- 서비스는 API를 실시간 조회하는 것이 아닌 DB에 저장된 데이터를 사용합니다.
- 행사 고유 식별자인 Identitykey를 기준으로 중복 저장을 방지했습니다.
- SyncStatus 엔티티를 통해 마지막 성공 동기화 시간을 관리하도록 구성했습니다.
...
#### 해결
- Spring Scheduler를 이용해 행사 데이터를 주기적으로 수집하도록 구현했습니다.
- 수집한 데이터를 DB에 저장해 백업 데이터로 활용할 수 있도록 구성했습니다.
- SHA-256 기반 identitykey를 사용해 중복 저장을 방지했습니다.
- 마지막 성공 동기화 시간을 별도로 저장해 데이터 최신 여부를 확인할 수 있도록 구현했습니다.
